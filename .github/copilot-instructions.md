...
- Parses query from message text using `text.split(" ").slice(1).join(" ").trim()`
...
- `ffmpeg-static`: Used for format conversion
...
## Key Files
- `index.js`: Message routing and connection logic
- `commands/play.js`: yt-dlp audio download example
